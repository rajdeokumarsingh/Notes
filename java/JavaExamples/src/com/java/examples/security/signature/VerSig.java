...
public class VerSig {
	...
	private void loadSignature() throws FileNotFoundException,
			IOException {
	...
	private void loadPublicKey() throws FileNotFoundException, IOException,
		 ...
		 * 
		 * KeyFactory class provides conversions between 
		 * 		opaque keys (of type Key) and key specifications
		 * 
		 * key specifications
		 * 		transparent representations of the underlying key material
		 * 
		 * With an opaque key 
		 * 		you can obtain the algorithm name, format name, 
		 * 		and encoded key bytes, but not the key material
		 * 			which may consist of the key itself 
		 * 				and the algorithm parameters used to calculate the key
		 * 
		 * first you need a key specification
		 * 		assuming that the key was encoded according to the X.509 standard
		 * 		 which is the case, for example, if the key was generated with 
		 * 		the built-in DSA key-pair generator supplied by the SUN provider
		 */
