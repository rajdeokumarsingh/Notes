...
CachedImage* CachedResourceLoader::requestImage(const String& url)
{
    ...
    // PEKALL begin: If user blocks image loading, we just return nothing.
    // Sometimes, the resource contains cached image which 
    // will be also displayed to user after "Block Image Loading" 
    // is turned on.
    if(!autoLoadImages()) return 0;
    // PEKALL end
