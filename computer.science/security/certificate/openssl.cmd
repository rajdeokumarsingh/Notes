# openssl config in mac /opt/local/etc/openssl/openssl.cnf
# change ./demoCA to .

# 如果发送错误： wrong number of fields on line 1 (looking for field 6, got 1, '' left)
# 清空index.txt文件
...
# openssl pkcs12 -export -clcerts -in certs/ca.cer -inkey private/server.key.pem -out certs/server.trust.p12

echo 颁发服务器证书
openssl genrsa -des3 -out private/server.key.pem 2048
...
# openssl pkcs12 -export -clcerts -in certs/server.cer -inkey private/server.key.pem -out certs/server.trust.p12
...
556  keytool -list -v -keystore ca.trust.jks
