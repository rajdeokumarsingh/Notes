...
public final class DownloadProvider extends ContentProvider {
    ...
    private static final String LOGTAG = "DownloadProvider";
    ...
    public Uri insert(final Uri uri, final ContentValues values) {
        ...
        // Hacked for wifi and 3g network switch
        String url = values.getAsString(Downloads.Impl.COLUMN_URI);
        Log.d(LOGTAG, "goto download url: " + url);
        if("browser://start_download_service/".equals(url)) {
            Log.d(LOGTAG, "goto download url start service: " + url);

            ContentValues v = new ContentValues();
            v.put(Constants.FAILED_CONNECTIONS, 0);
            String failedCountWhere = Constants.FAILED_CONNECTIONS + "!= 0";
            db.update(DB_TABLE, v, failedCountWhere, null);
