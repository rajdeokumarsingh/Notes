...
public class DownloadAdapter extends CursorAdapter {
    ...
    public void bindView(View convertView, int position) {
        ...
        TextView progress_text = (TextView)convertView.findViewById(R.id.progress_text);
        View progress = convertView.findViewById(R.id.download_progress);
        View size_text = convertView.findViewById(R.id.size_text);
        View status_text = convertView.findViewById(R.id.status_text);
        View last_modified_date = convertView.findViewById(R.id.last_modified_date);
        View domain = convertView.findViewById(R.id.domain);
        ...
        if (isStatusCompleted(status)) { 
            ...
            last_modified_date.setVisibility(View.VISIBLE);
            ...
            last_modified_date.setVisibility(View.GONE);
            if (status == DownloadManager.STATUS_PENDING) {
                 progress_text.setText(mResources.getText(R.string.download_pending));
             } else if (status == DownloadManager.STATUS_PAUSED) {
                 ProgressBar pb = (ProgressBar) progress;
                 StringBuilder sb = new StringBuilder();
                 sb.append(mResources.getText(R.string.download_pending_network));
                 if (totalBytes > 0) {
                     ...
                     sb.append("% (");
                     sb.append(Formatter.formatFileSize(mDownloadList, currentBytes));
                     sb.append("/");
                     sb.append(Formatter.formatFileSize(mDownloadList, totalBytes));
                     sb.append(")");
                 ...
                 if (status == DownloadManager.STATUS_RUNNING) {
                     sb.append(mResources.getText(R.string.download_running));
                 } else {
                     sb.append(mResources.getText(R.string.download_running_paused));
                 }
                 if (totalBytes > 0) {
                     long currentBytes = mCursor.getLong(mCurrentBytesColumnId);
                     int progressAmount = (int)(currentBytes * 100 / totalBytes);
    ...
    private int getStatusStringId(int status) {
        switch (status) {
            case DownloadManager.STATUS_FAILED:
                String errorMsg = mCursor.getString(mErrorMsg);

                if (errorMsg != null &&
                        (errorMsg.contains("is below 10% availability") ||
                        errorMsg.contains("unable to free any more")))
                    return R.string.not_enough_space_error;
