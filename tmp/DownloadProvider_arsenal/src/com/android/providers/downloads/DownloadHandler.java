...
public class DownloadHandler {
    ...
    synchronized void dequeueDownload(long mId) {
        if (Constants.LOGV) {
            Log.i(TAG, "dequeueDownload, id: " + mId);
        }
        mDownloadsInProgress.remove(mId);
        startDownloadThread();
        if (mDownloadsInProgress.size() == 0 && mDownloadsQueue.size() == 0) {
            if (Constants.LOGV) {
                Log.i(TAG, "dequeueDownload, no pending download");
            }
            notifyAll();
        }
    }

    synchronized boolean hasPendingDownload() {
        return (mDownloadsInProgress.size() != 0 || mDownloadsQueue.size() != 0);
    }
