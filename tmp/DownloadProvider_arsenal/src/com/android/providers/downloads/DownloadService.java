...
public class DownloadService extends Service {
            ...
            for (;;) {
                ...
                synchronized (mDownloads) {
                    ...
                    try {
                        ...
                        if (Constants.LOGVV) {
                            Log.i(TAG, "number of rows from downloads-db: " +
                                    cursor.getCount());
                        }
    ...
    private void updateDownload(DownloadInfo.Reader reader, DownloadInfo info, long now) {
        ...
        if (Constants.LOGVV) {
            Log.v(TAG, "processing updated download " + info.mId +
                    ", status: " + info.mStatus);
        }
        ...
        synchronized (this) {
            if (mMediaScannerService == null) {
                ...
                while (mMediaScannerConnecting) {
                    Log.d(TAG, "waiting for mMediaScannerService service: ");
                    try {
            ...
            try {
                ...
                Log.w(TAG, "Failed to scan file " + info.mFileName);
                return false;
            }
        }
    }
    ...
    private void deleteFileIfExists(String path) {
        try {
            ...
            Log.w(TAG, "file: '" + path + "' couldn't be deleted", e);
        }
    }
    ...
    private class ApnHelper {
        ...
        int networkToApnType(String network) {
            if("mobile_internet".equals(network))
                return DownloadManager.Request.NETWORK_MOBILE_INTERNET;
            if("mobile_wap".equals(network))
                return DownloadManager.Request.NETWORK_MOBILE_WAP;
        ...
        String getApnFeatureName(int networkType) {
            if(networkType == DownloadManager.Request.NETWORK_MOBILE_INTERNET)
                return "enableINTERNET";

            if(networkType == DownloadManager.Request.NETWORK_MOBILE_WAP)
                return "enableWAP";
        ...
        void beginDownload(DownloadInfoNow din) {
            ...
            try {
                if(!Helpers.isSpecialNetworkType(info.mAllowedNetworkTypes)) {
                    if(mFeatureUp)
                        endConnectivity(DownloadService.this, mFeatureName);
