...
public class BrowserSettings implements OnSharedPreferenceChangeListener,
  ...
  //pekall
    ...
    public void syncSharedPreferences(){
        try {
            Log.v(LOGTAG, "enter syncSharedPreferences");
    
            dataConnection = (String)getApnType(mPrefs.getString(
                        PREF_DATA_CONNECTION, "CMNET"));
    ...
    private void syncSetting(WebSettingsClassic settings) {
        ...
        settings.setLoadsImagesAutomatically(checkImageState());
    ...
    private boolean checkImageState(){
    	int picture = mPrefs.getInt(PREF_PAGE_PICTURE, 1);
    	if(picture == 0){
    		return false;
    	}else if(picture == 1){
    		return true;
...
//        }
   ...
   public static final boolean FORCE_SYS_CONN = isForceSystemConnection();
    ...
    private static final boolean isCuPlatform() {
        ...
        // Coolpad CP7728
        if(product != null && product.contains("CP7728"))
            return true;
    ...
    private static final boolean isCmccPlatform() {
        ...
        // Lenovo S899T
        if(product != null && product.contains("S899t"))
            return true;

        // Huawei T8808D
        if(product != null && product.contains("T8808D"))
            return true;
