...
public class BookmarkUtils {
    ...
    static Drawable createListFaviconBackground(Context context) {
        ...
        faviconBackground.getPaint().setColor(Color.TRANSPARENT);
    ...
    static void displayRemoveBookmarkOrFolderDialog( final long id, final String title,
            final Context context, final Message msg, final boolean isFolder) {
        String message = null;
        if(isFolder){
            message = context.getString(R.string.delete_folder_warning,title);
        }else{
            message = context.getString(R.string.delete_bookmark_warning,title);
        }
        ...
        .setPositiveButton(R.string.ok,
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int whichButton) {
                        ...
                        if(isFolder){
                            Runnable folder = new Runnable(){
                                @Override
                                public void run() {
                                    context.getContentResolver().delete(BrowserContract.Bookmarks.CONTENT_URI, "_id = ? or parent = ?", new String[]{Long.toString(id) , Long.toString(id)});
                                }
                            };
                            new Thread(folder).start();
                        } else {
                            Runnable runnable = new Runnable(){
                                @Override
                                public void run() {
