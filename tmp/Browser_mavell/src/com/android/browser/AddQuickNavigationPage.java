...
public class AddQuickNavigationPage extends Activity implements OnClickListener {
    ...
    private void save() {
        ...
        if(add){
            Cursor cursor = getContentResolver().query(BrowserProvider2.NavScreen.CONTENT_URI, BrowserProvider2.SCREEN_PROJECTION, "url = ?", new String[]{mUrl}, null);
            ...
            add.put(NavScreen.TITLE, this.getString(R.string.add_new_quick_navigation));
            add.put(NavScreen.UPDATE, 1);
            add.put(NavScreen.DATE, now);
            getContentResolver().insert(BrowserProvider2.NavScreen.CONTENT_URI, add);
        }else {
            ContentValues values = new ContentValues();
            values.put(BrowserProvider2.NavScreen.TITLE, mTitle);
            if(urlChange){
