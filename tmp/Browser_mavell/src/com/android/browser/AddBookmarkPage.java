...
public class AddBookmarkPage extends Activity
 ...
 //               finish();
    ...
    boolean save() {
        ...
        if(folderId > 0){
            ...
            if (mEditingExisting) {
                ...
                BookmarkReplaceAlertDialogFragment.newInstance(editId, title, finalUrl, mCurrentFolder, false).show(getFragmentManager(), "replace");
                setResult(RESULT_OK);
			}
	        else if (bookmarkId > 0){
                BookmarkReplaceAlertDialogFragment.newInstance(bookmarkId, title, finalUrl, mCurrentFolder, true).show(getFragmentManager(), "replace");
                setResult(RESULT_OK);
	        } else {
	            Bitmap thumbnail;
	            Bitmap favicon;
    ...
    public static class BookmarkReplaceAlertDialogFragment extends DialogFragment {
        ...
        public static BookmarkReplaceAlertDialogFragment newInstance(int bookmarkId, String title, String finalUrl, long mCurrentFolder, boolean add) {
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            int dialogMessage = R.string.replace_bookmark_tip;
            if(add){
                dialogTitle = R.string.add_bookmark_short;
                dialogMessage = R.string.add_replace_bookmark_tip;
            }
            return new AlertDialog.Builder(getActivity(), AlertDialog.THEME_HOLO_LIGHT).setTitle(dialogTitle).setMessage(
                    dialogMessage)
					.setNegativeButton(R.string.cancel,
						...
						public void onClick(DialogInterface dialog, int which) {
							((AddBookmarkPage)getActivity()).updateBookmark(bookmarkId, title, finalUrl, mCurrentFolder);
						}
					})
                    .create();
        }
    ...
    //pekall update
	private void updateBookmark(int bookmarkId, String title, String url, long folderId) {
		createHandler();
//		Bundle bundle = new Bundle();
