...
public class Controller
    ...
    private static int LastExtremeIndex = -1;
    private ThemeCheck mThemeCheck = new ThemeCheck();

    // public message ids
    public final static int LOAD_URL = 1001;
    public final static int STOP_LOAD = 1002;
    private static final int GO_WIFI_SETTING = 1003;
    ...
    final static int EDIT_QUICK_NAVIGATION = 7;
    final static int ROTARY = 8;
    final static int FLIP = 9;
    ...
    class CmccWifiHandler {
        ...
        void setUseWifi(boolean useWifi) {
            if(ENABLE_LOG) Log.v(LOGTAG, "setUseWifi(): " + useWifi);
            this.mUseWifi = useWifi;
        }

        void setUserConfigWifi(boolean b) {
            if(ENABLE_LOG) Log.v(LOGTAG, "setUserConfigWifi(): " + b);
            mUserConfigWifi = b;
        }

        boolean userConfigWifi() {
            if(ENABLE_LOG) Log.v(LOGTAG, "enter userConfigWifi(): " + mUserConfigWifi);
            return mUserConfigWifi;
        }

        // WIFI is power-on, but NO APs nearby
        boolean isWifiPowerOn() {
            WifiManager mng = (WifiManager) mActivity.
                    getSystemService(Context.WIFI_SERVICE);
            final List<ScanResult> results = mng.getScanResults();

            if(ENABLE_LOG) Log.v(LOGTAG, "enter isWifiPowerOn(): wifi enable: " +
                    mng.isWifiEnabled() + ", ScanResult: " + results);
            return mng.isWifiEnabled() && (results == null || results.isEmpty());
        }

        // WIFI power-on, and found APs nearby
        boolean isWifiEnabled() {
            WifiManager mng = (WifiManager) mActivity.getSystemService(Context.WIFI_SERVICE);
            final List<ScanResult> results = mng.getScanResults();

            if (ENABLE_LOG) Log.v(LOGTAG, "enter isWifiEnabled(): wifi enable: " +
                    mng.isWifiEnabled() + ", ScanResult: " + results);
        ...
        boolean isWifiActivated() {
            if(ENABLE_LOG) Log.v(LOGTAG, "enter isWifiActivated()");
            return (isWifiEnabled() && !isWifiConnected());
        }
    }

    private CmccWifiHandler mCmccWifiHandler;

    private void handleApnResume() {
        if (mCmccWifiHandler.isWifiActivated()) {
            if (ENABLE_LOG) Log.v(LOGTAG, "onResume, wifi is activated");
            if (mCmccWifiHandler.userConfigWifi()) {
                // Clear the flag
                if (ENABLE_LOG) Log.v(LOGTAG, "onResume, wifi is activated, user has gone to wifi settings");
            } else {
                if (ENABLE_LOG) Log.v(LOGTAG, "onResume, wifi is activated, go wifi settings");
                mHandler.sendEmptyMessage(GO_WIFI_SETTING);
            }
            mCmccWifiHandler.setUserConfigWifi(true);
            return;
        }

        if (mCmccWifiHandler.isWifiConnected()) {
            if (ENABLE_LOG) Log.v(LOGTAG, "onResume, use wifi");

            mCmccWifiHandler.setUseWifi(true);
            // TODO: check weather need to display this message
            // Toast.makeText(mActivity, R.string.wifi_connected, Toast.LENGTH_SHORT).show();
            ...
            if ((mNetworkHandler.isNetworkUp() && mCmccWifiHandler.isUseWifi()) ||
                    (mCmccWifiHandler.isWifiPowerOn())) {
    ...
    public Controller(Activity browser) {
        ...
        mVoiceFlip = mSettings.getSharedPreferences().getBoolean(PreferenceKeys.PREF_VOICE_FLIP, false);
        ...
        mCmccWifiHandler = new CmccWifiHandler();
    }
    ...
    private void init() {
        ...
        pageUp.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                // TODO Auto-generated method stub
                getCurrentTab().getWebView().pageUp(false);
            }
        });
        ...
        pageDown.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                // TODO Auto-generated method stub
                getCurrentTab().getWebView().pageDown(false);
            }
        });
    ...
    @Override
    public void setVoiceFlip(boolean voice){
    	mVoiceFlip = voice;
    }
...
//        		mTabControl.canRestoreState(icicle, restoreIncognitoTabs);
    ...
    public void onPause() {
        ...
        //when home key, onPause will pause JS execute, check if there are tabs' url nneded Cancel Extreme Booster
        int inactiveTabs = mTabControl.getTabCount() - 1;
        String url = "";
        for (int i = inactiveTabs; i >= 0; i--) {
             url = mTabControl.getTab(i).getUrl();

            if(mThemeCheck.isThemeExtreme(url) >= 0) {
                Context cur_context = getContext();
                mThemeCheck.trySendFinishBoosterIntent(false, cur_context);
            }
        }
    ...
    public void onResume() {
        ...
        if (current != null) {
            ...
            //When Resume, if current tab's url is Extreme booster, do Booster Policy
            int extreme_index = mThemeCheck.isThemeExtreme(current.getUrl());
            if(extreme_index >= 0) {
                Context cur_context = getContext();
                mThemeCheck.SendBoosterIntent(false, cur_context, extreme_index);
            }
        }
        releaseWakeLock();
    ...
    public void onPageStarted(Tab tab, WebView view, Bitmap favicon) {
        ...
        Tab cuttent = getCurrentTab();
        if(cuttent != null){
	        if(cuttent.canGoForward()){
        ...
        //Booster loading when url is normal
        int extreme_index = mThemeCheck.isThemeExtreme(url);
        if(extreme_index < 0) {
            Context cur_context = getContext();
            mThemeCheck.SendBoosterIntent(true, cur_context, 0);

            //for Extreme, current policy is reverse, when url finish loading, booster it 140s,
            //when we finish run and start show result, Cance Extreme booster
            //here is try to finish Extreme booster
            if(LastExtremeIndex >= 0) {
                mThemeCheck.trySendFinishBoosterIntent(false, cur_context);
            }
        }
    ...
    public void onPageFinished(Tab tab) {
        ...
        //currently, if Extreme loading finish, we need booster run
        int extreme_index = mThemeCheck.isThemeExtreme(url);
        if(extreme_index >= 0) {
            Context cur_context = getContext();
            mThemeCheck.SendBoosterIntent(false, cur_context, extreme_index);
        }
        LastExtremeIndex = extreme_index;

        //if normal url loading finish, try cancel normal booster
        if(url.equals(LastUrl)) {
            Context cur_context = getContext();
            mThemeCheck.trySendFinishBoosterIntent(true, cur_context);
        }
                ...
                if (Intent.ACTION_VIEW.equals(intent.getAction())) {
                    ...
                    for (String url : urls) {
                        parent = openTab(url, parent,
                                !mSettings.openInBackground(), false);
                    }
    ...
    public void onCreateContextMenu(ContextMenu menu, View v,
        ...
        menu.setGroupVisible(R.id.ANCHOR_MENU,
                ...
                || type == WebView.HitTestResult.SRC_IMAGE_ANCHOR_TYPE);
        boolean hitText = type == WebView.HitTestResult.PHONE_TYPE
                || type == WebView.HitTestResult.EMAIL_TYPE
                || type == WebView.HitTestResult.GEO_TYPE;
...
//        }
    ...
    public void updateMenuState(Tab tab, Menu menu) {
        ...
        final MenuItem home = menu.findItem(R.id.homepage2_menu_id);
...
//                break;
    ...
    public void setActiveTab(Tab tab) {
        ...
        if (tab != null) {
            ...
            int extreme_index = mThemeCheck.isThemeExtreme(tab.getUrl());
            if(extreme_index >= 0) {
                Context context = getContext();
                mThemeCheck.SendBoosterIntent(false, context, extreme_index);
            }
        }
    }
    ...
    public Tab openTabToHomePage() {
        ((BaseUi)mUi).hideHomePage();
        MainMenuControlPhone phone = ((BaseUi)mUi).getMainMenuControlPhone();
        if(phone!=null && phone.getTabMenu()!=null && phone.getTabMenu().isShowing()){
        	phone.hideMenu();
        }
    ...
    protected void closeCurrentTab(boolean andQuit) {
        ...
        Context cur_context = getContext();
        mThemeCheck.checkTabCancelBooster(current, cur_context, LastUrl);
    ...
    public void closeTab(Tab tab) {
        Context cur_context = getContext();
        mThemeCheck.checkTabCancelBooster(tab, cur_context, LastUrl);
    ...
    protected void loadUrl(Tab tab, String url, Map<String, String> headers) {
        ...
        int flip = mSettings.getSharedPreferences().getInt(PreferenceKeys.PREF_PAGE_FLIP, 0);
        if((flip==1||flip==2) && !pageFilpState){
            addPageFlip();
        }
    ...
    void goBackOnePageOrQuit() {
        final Tab current = mTabControl.getCurrentTab();
        //when Extreme booster run and Back key is clicked,
        //try to cancel this Extreme Booster
...
//                    mHandler.removeMessages(LOAD_URL_DELAY);
                    ...
                    new AlertDialog.Builder(mActivity, AlertDialog.THEME_HOLO_LIGHT)
                        ...
                        .setPositiveButton(R.string.yes, new DialogInterface.OnClickListener() {
                            ...
                            .show();
                            **/
                    if(System.currentTimeMillis() - goBackTime <= 5000){
                        if ((current.getAppId() != null) || current.closeOnBack()) {
                            if(ENABLE_LOG) 
                                Log.v(LOGTAG, "enter goBackOnePageOrQuit(), closeCurrentTab");
                            closeCurrentTab(true);
                        ...
                        toast = Toast.makeText(mActivity, R.string.exit_browser_comfirm, Toast.LENGTH_SHORT);
                        toast.show();
                    }
...
//            }
            ...
            Log.v("FileManager", "KEYCODE_HOME");
            if (tabMenu != null) { 
                if (tabMenu.isShowing()){ 
                    Log.v("FileManager", "KEYCODE_HOME 2");
                    tabMenu.dismiss(); 
                }
            } 
            return true;
