...
public class IntentHandler {
    ...
    void onNewIntent(Intent intent) {
        ...
        if (Intent.ACTION_VIEW.equals(action)
            ...
            if(intent != null &&Intent.ACTION_SEARCH.equals(action))
            {
                String url = intent.getStringExtra(SearchManager.QUERY);
                if(url != null && url.toLowerCase().startsWith("rtsp://")) {
                    ...
                    try {
                        SendIntent = Intent.parseUri(url, Intent.URI_INTENT_SCHEME);
                    } catch (URISyntaxException ex) {
                        Log.w("Browser", "Bad URI " + url + ": " + ex.getMessage());
                        return;
                    }
                    try {
                        if (mActivity.startActivityIfNeeded(SendIntent, -1)) {
