...
import rds.app.Application;
import rds.app.InHouse;
import rds.app.Paging;
import rds.app.oda.AlertC;
import rds.app.oda.ODA;
...
import rds.log.ApplicationChanged;
import rds.log.ClockTime;
import rds.log.EONReturn;
import rds.log.EONSwitch;
import rds.log.GroupReceived;
import rds.log.Log;
import rds.log.RDSTime;
import rds.log.StationLost;
import rds.log.StationTuned;

public class GroupLevelDecoder {
    private static final String LOGTAG = "rds.GroupLevelDecoder";
    ...
    private void processGroup(int nbOk, boolean[] blocksOk, int[] blocks, RDSTime time) {
        ...
        if(type == 2 && (blocksOk[2] || blocksOk[3])) {
            ...
            // FIXME: Jiang Rui, just for test a group
            ...
            Text rt = (ab == 0) ? workingStation.getRT() :
                workingStation.getRTVersionB();
        ...
        android.util.Log.i(LOGTAG, "decoder log: " +
    ...
    private final GroupReaderEventVisitor readerEventVisitor = new GroupReaderEventVisitor() {
        ...
        public void visit(GroupEvent groupEvent) {
            ...
            int nbOk = 0;
            for(int i=0; i<4; i++) {
                // FIXME: Jiang Rui, the blocks[i] is a 16-bit short
