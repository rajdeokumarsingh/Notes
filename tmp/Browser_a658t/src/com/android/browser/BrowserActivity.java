...
public class BrowserActivity extends Activity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        StatFs memory = new StatFs(Environment.getDataDirectory().toString());
        int availableBlocks = memory.getAvailableBlocks();
        int blockSize = memory.getBlockSize();
        if (availableBlocks * blockSize < 1000000) {
            Log.e(LOGTAG, "BrowserActivity.onCreate: not enough memory");
            Toast.makeText(this, R.string.not_enough_memory_space, Toast.LENGTH_LONG).show();
            finish();
        }
...
/*        long startRxBytes = TrafficStats.getUidRxBytes(getAppUid());
        long startTxBytes = TrafficStats.getUidTxBytes(getAppUid());
        SharedPreferences mPrefs = PreferenceManager.getDefaultSharedPreferences(this);
        mPrefs.edit().putLong(PreferenceKeys.PREF_FLOW_START_RX, startRxBytes).putLong(PreferenceKeys.PREF_FLOW_START_TX, startTxBytes).commit();**/
    ...
    protected void onResume() {
        ...
        int blockSize = memory.getBlockSize();
        if (availableBlocks * blockSize < 1000000) {
            Log.e(LOGTAG, "BrowserActivity.onResume: not enough memory");
            Toast.makeText(this, R.string.not_enough_memory_space, Toast.LENGTH_LONG).show();
            finish();
        }
        if (mController != null) {
