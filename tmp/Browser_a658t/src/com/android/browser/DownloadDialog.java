...
public class DownloadDialog extends Activity implements View.OnClickListener{
    ...
    public static final String IMADE_DIR = "Picture";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(contentType==1){
        	...
        	file_path_text = download_path+"/"+APPLICATION_DIR;
        }else {
            file_path_text = download_path+"/"+OTHERS_DIR;
        }
...
//                DownloadDialog.this.startActivityForResult(intent, FILE_RESULT);  
                Intent intent = new Intent(this, FileManagerMainActivity.class);
            ...
            case R.id.ok:
                ...
                onDownloadStartNoStream(url, mimetype, privateBrowsing, userAgent);
                break;        
            default:
                break;
        }
    ...
    void onDownloadStartNoStream(String url, String mimetype, boolean privateBrowsing, String userAgent){
        ...
        if(BrowserSettings.CMCC_PLATFORM) {
            ...
            Log.v(LOGTAG, "DOWNLOAD APN:" + apn);

            int type = ~0; // allow all network type
            final int NETWORK_MOBILE_WAP = 1 << 2;
            final int NETWORK_MOBILE_INTERNET = 1 << 3;
            ...
            if(isWifiConnected()){
                ...
                type = NETWORK_MOBILE_WAP;
            } else if("internet".equals(apn)) {
                // FIXME: just_for build
                // type= Request.NETWORK_MOBILE_INTERNET;
                type= NETWORK_MOBILE_INTERNET;
            } else if("system".equals(apn)){
            	String system = mSettings.getSystemApnName();
            	Log.v(LOGTAG, "CMCC DOWNLOAD SYSTEM APN:"+system);
            	if(system.contains("CMWAP")){
            		 type = NETWORK_MOBILE_WAP;
            	}else if(system.contains("CMNET")){
            		type= NETWORK_MOBILE_INTERNET;
            	}
            }
...
//            root = Environment.getExternalStorageDirectory_ext().getPath();
    ...
    static void createFiles(String download_path){
         File downloadDir = getFile(download_path+"/"+OTHERS_DIR);
         ...
         File applicationDir = getFile(download_path+"/"+APPLICATION_DIR);
    }
