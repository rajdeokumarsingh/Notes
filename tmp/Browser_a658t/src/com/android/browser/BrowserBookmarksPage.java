...
public class BrowserBookmarksPage extends Fragment implements View.OnCreateContextMenuListener,
    ...
    public boolean handleContextItem(int itemId, int groupPosition,
        ...
        case R.id.move_context_menu_id:
            moveBookmark(adapter, childPosition);
            break;
...
//pekall
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
    	super.onCreateOptionsMenu(menu, inflater);
           inflater.inflate(R.menu.bookmarksmenu, menu);
     };
 
    @Override
    public void onPrepareOptionsMenu(Menu menu) {
//    	menu.findItem(R.id.delete_bookmarks).setVisible(mAdapter.getCursorCount()>0);
     }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.create_bookmark:
			 Intent bookmarkIntent = createBookmarkIntent(false);
             if (bookmarkIntent != null) {
               startActivity(bookmarkIntent);
            }
			break;
//		case R.id.delete_bookmarks:
//			Intent d = new Intent(this, FolderBookmarksDeletePage.class);
//			d.putExtras(getIntent());
//			startActivityForResult(d, BOOKMARKS_SAVE);
//			break;
    ...
    public Intent createBookmarkIntent(boolean editExisting) {
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
        ...
        if(BrowserSettings.CU_PLATFORM){
            id = R.menu.bookmarkscontext_cu;
        }else {
            id = R.menu.bookmarkscontext;
        }
    ...
    private void editFolder(){
        ...
        item.putLong(BrowserContract.Bookmarks.PARENT,
        		defaultFolder);
        item.putBoolean(saveFolder, true);
    ...
    private void editBookmark(BrowserBookmarksAdapter adapter, int position) {
        ...
        byte[] data2 = cursor.getBlob(BookmarksLoader.COLUMN_INDEX_TOUCH_ICON);
        if (data2 != null) {
            item.putParcelable(BrowserContract.Bookmarks.TOUCH_ICON,
                    BitmapFactory.decodeByteArray(data2, 0, data2.length));
        }
        item.putLong(BrowserContract.Bookmarks._ID,
                cursor.getLong(BookmarksLoader.COLUMN_INDEX_ID));
        item.putBoolean("edit", true);
        item.putLong(BrowserContract.Bookmarks.PARENT,
    ...
    private void moveBookmark(BrowserBookmarksAdapter adapter, int position) {
        Intent intent = new Intent(getActivity(), AddBookmarkPage.class);
        Cursor cursor = adapter.getItem(position);
        Bundle item = new Bundle();
        item.putString(BrowserContract.Bookmarks.TITLE,
        ...
        item.putBoolean(AddBookmarkPage.EXTRA_MOVE_BOOKMARK, true);
        item.putLong(BrowserContract.Bookmarks._ID,
                cursor.getLong(BookmarksLoader.COLUMN_INDEX_ID));
        item.putLong(BrowserContract.Bookmarks.PARENT,
                cursor.getLong(BookmarksLoader.COLUMN_INDEX_PARENT));
    ...
    private void displayRemoveBookmarkDialog(BrowserBookmarksAdapter adapter,
        ...
        BookmarkUtils.displayRemoveBookmarkOrFolderDialog(id, title, context, null, isFolder);
    }
