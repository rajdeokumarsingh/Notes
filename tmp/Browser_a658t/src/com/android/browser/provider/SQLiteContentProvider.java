...
public abstract class SQLiteContentProvider extends ContentProvider {
    ...
    private static final String LOGTAG_SYNC = "SQLiteContentProvider_sync";
    ...
    protected void postNotifyUri(Uri uri) {
        synchronized (mChangedUris) {
            if(Browser.LOG_SYNC_ENABLED) {
                Log.i(LOGTAG_SYNC, "postNotifyUri()," + 
                        " synchronized(mChangedUris), IN");
            }
            mChangedUris.add(uri);
            if(Browser.LOG_SYNC_ENABLED) {
                Log.i(LOGTAG_SYNC, "postNotifyUri()," + 
                        " synchronized(mChangedUris), OUT");
            }
        }
    }
    ...
    protected void onEndTransaction(boolean callerIsSyncAdapter) {
        ...
        synchronized (mChangedUris) {
            if(Browser.LOG_SYNC_ENABLED) {
                Log.i(LOGTAG_SYNC, "onEndTransaction()," + 
                        " synchronized(mChangedUris), IN");
            }
