...
public class GoogleAccountLogin implements Runnable,
    ...
    private static final String LOGTAG_SYNC = "GoogleAccountLogin_sync";
                ...
                synchronized (GoogleAccountLogin.this) {
                    if(Browser.LOG_SYNC_ENABLED) {
                        Log.i(LOGTAG_SYNC, "run()," + 
                                " synchronized(GoogleAccountLogin.this), IN");
                    }
                    if (mRunnable == null) {
                        if(Browser.LOG_SYNC_ENABLED) {
                            Log.i(LOGTAG_SYNC, "run()," + 
                                    " synchronized(GoogleAccountLogin.this), OUT 1");
                        }
                        return;
                    }
                    ...
                    if(Browser.LOG_SYNC_ENABLED) {
                        Log.i(LOGTAG_SYNC, "run()," + 
                                " synchronized(GoogleAccountLogin.this), OUT 2");
                    }
                }
            }
        });
    ...
    private synchronized void done() {
        if(Browser.LOG_SYNC_ENABLED) {
            Log.i(LOGTAG_SYNC, "done()," + 
                    " synchronized done(), IN");
        }
        if (mRunnable != null) {
