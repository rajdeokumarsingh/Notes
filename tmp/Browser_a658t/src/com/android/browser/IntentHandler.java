...
public class IntentHandler {
    ...
    private static final String LOGTAG = "IntentHandler";
    ...
    void onNewIntent(Intent intent) {
        if(ENABLE_LOG) Log.v(LOGTAG, "enter onNewIntent()");
        ...
        if (Intent.ACTION_VIEW.equals(action)
            ...
            if(intent!=null){
            	 ...
            	 if(data!=null){
            	        if(data.toString().length() >= UrlInputView.LIMIT_URL_LENGTH){
            ...
            if (urlData.isEmpty()) {
                if(ENABLE_LOG) Log.v(LOGTAG, 
                        "enter onNewIntent(), go home page");
            ...
            if (intent.getBooleanExtra(Browser.EXTRA_CREATE_NEW_TAB, false)
                  || urlData.isPreloaded()) {
                if(ENABLE_LOG) Log.v(LOGTAG, 
                        "enter onNewIntent(), create new tab");
            ...
            if (!TextUtils.isEmpty(urlData.mUrl) &&
                ...
                if(ENABLE_LOG) Log.v(LOGTAG, 
                        "enter onNewIntent(), load javascript");
                mController.openTab(urlData);
                return;
            }

            if(ENABLE_LOG) Log.v(LOGTAG, 
                    "enter onNewIntent(), open tab" 
                    + ", action: " + action
                    + ", activateVoiceSearch: " + activateVoiceSearch
                    + ", appId: " + appId);
            ...
            if ((Intent.ACTION_VIEW.equals(action)
                ...
                if (appTab != null) {
                    if(ENABLE_LOG) Log.v(LOGTAG, 
                            "enter onNewIntent(), findTabWithUrl not null");
                    ...
                    if(Controller.homepage == true||Controller.homepageBack == true){
                        ...
                        if(ENABLE_LOG) Log.v(LOGTAG, 
                                "enter onNewIntent(), hideHomePage");
                        ((BaseUi)mController.getUi()).hideHomePage();
                    }
                    if (current != appTab) {
                        if(ENABLE_LOG) Log.v(LOGTAG, 
                                "enter onNewIntent(), switchToTab");
                        mController.switchToTab(appTab);
                    }
                    // Otherwise, we are already viewing the correct tab.
                } else {
                    if(ENABLE_LOG) Log.v(LOGTAG, 
                            "enter onNewIntent(), findTabWithUrl null");
        ...
        if (intent != null
            ...
            if (Intent.ACTION_VIEW.equals(action) ||
                ...
                if(intent.getData() != null) {
                    ...
                    if(UrlHandler.isSpecialUrl(url)) {
                        if(LOGV_ENABLED) Log.v(LOGTAG, "special url: " + url);
    ...
    static boolean handleWebSearchIntent(Activity activity,
        ...
        if(LOGV_ENABLED) Log.v(LOGTAG, "handleWebSearchIntent: " + url);

        if(UrlHandler.isSpecialUrl(url)) {
            if(LOGV_ENABLED) 
                Log.v(LOGTAG, "handleWebSearchIntent, special url ");
            return false;
        }
    ...
    private static boolean handleWebSearchRequest(Activity activity,
        ...
        if(url.length() >= UrlInputView.LIMIT_URL_LENGTH){
            Toast.makeText(activity, R.string.url_write_limit2, Toast.LENGTH_SHORT).show();
            activity.finish();
            return false;
        }
