...
public class AddBookmarkPage extends Activity
    ...
    static final String EXTRA_MOVE_BOOKMARK = "move_bookmark";
    static final String EXTRA_EDIT_BOOKMARK_E = "edit";   
 ...
 //               finish();
    ...
    private LoaderCallbacks<EditBookmarkInfo> mEditInfoLoaderCallbacks =
        ...
        public void onLoadFinished(Loader<EditBookmarkInfo> loader,
            ...
            if (info.id != -1) {
                ...
                }else if(moveBookmark){
                    mFakeTitle.setText(R.string.move_bookmark);
                }else{
                    mFakeTitle.setText(R.string.edit_bookmark);
                }

                if(!mPaused) {
                    Log.i(LOGTAG, "onLoadFinished, has paused, not set title");
    ...
    private boolean moveBookmark = false;
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (mMap != null) {
            ...
            if (b != null) {
                ...
                mMap = b;
                saveFolder = mMap.getBoolean(BrowserBookmarksPage.saveFolder,false);
                moveBookmark = mMap.getBoolean(EXTRA_MOVE_BOOKMARK);
                editBookmark = mMap.getBoolean(EXTRA_EDIT_BOOKMARK_E);
                mEditingExisting = true;
                mFakeTitle.setText(R.string.edit_bookmark);
                if (mEditingFolder) {
                	if(saveFolder){
            ...
            mOriginalId = (int)mMap.getLong("_id", -1);
        }
    ...
    private class SaveBookmarkRunnable implements Runnable {
        ...
        public void run() {
            ...
            int bookmarmId = bundle.getInt("bookmarkId", -1);
            Long folderId = bundle.getLong("parent");
            boolean delete = bundle.getBoolean("delete", false);
            int folder = bundle.getInt(EXTRA_IS_FOLDER,0);
            ...
            try {
                ...
                if(bookmarmId > 0){
                    if(delete){
                        Bookmarks.deleteBookmark(AddBookmarkPage.this, url, bookmarmId);
                    }else{
                        Bookmarks.updateBookmark(null, cr,bookmarmId, url, title, thumbnail, true, folderId, folder);
                    }
                }else{
                	Bookmarks.addBookmark(AddBookmarkPage.this, false, url,
                            title, thumbnail, mCurrentFolder, folder);
                }
    ...
    boolean save() {
        ...
        if (!saveFolder && !mEditingFolder) {
            if (!Patterns.WEB_URL.matcher(unfilteredUrl).matches()
                    && !UrlUtils.ACCEPTED_URI_SCHEMA.matcher(unfilteredUrl).matches()){
                mAddress.setError(r.getText(R.string.match_url));
                return false;
            }
        }
        ...
        if (mSaveToHomeScreen) {
            ...
            Intent i = BookmarkUtils.createAddToHomeIntent(AddBookmarkPage.this, finalUrl, title, touchIcon, favicon);
        ...
        if(saveFolder){
        	folderId = Bookmarks.isFolderExisting(getContentResolver(), title, mCurrentFolder, mOriginalId);
        }
        if(folderId > 0){
        	Toast.makeText(AddBookmarkPage.this, 
                    R.string.folder_exist, Toast.LENGTH_LONG).show();
        }else{
        	//isBookmarkExisting
            int bookmarkId = 0;
            if(!saveFolder){
                bookmarkId = Bookmarks.isBookmarkExisting(getContentResolver(), title, url, mCurrentFolder, mOriginalId);
            }
            if (mEditingExisting) {
                Long id = mMap.getLong(BrowserContract.Bookmarks._ID);
                int editId = id.intValue();
                String finalUrls = null;
                if(!mEditingFolder){
                    finalUrls = finalUrl;
                }
                if(moveBookmark && bookmarkId>0){
                    BookmarkReplaceAlertDialogFragment.newInstance(editId, bookmarkId, title, finalUrl, mCurrentFolder, false, true, false, false).show(getFragmentManager(), "replace");
                }else if(moveBookmark){
                    BookmarkReplaceAlertDialogFragment.newInstance(editId, 0, title, finalUrl, mCurrentFolder, false, false, true, false).show(getFragmentManager(), "replace");
                }else if(editBookmark && bookmarkId>0){
                    BookmarkReplaceAlertDialogFragment.newInstance(editId, 0, title, finalUrl, mCurrentFolder, false, true, false, true).show(getFragmentManager(), "replace");
                }else if(editBookmark){
                    BookmarkReplaceAlertDialogFragment.newInstance(editId, 0, title, finalUrl, mCurrentFolder, false, false, false, true).show(getFragmentManager(), "replace");
                }else {
                    BookmarkReplaceAlertDialogFragment.newInstance(editId, 0, title, finalUrl, mCurrentFolder, false, false, false, false).show(getFragmentManager(), "replace");

                }
                setResult(RESULT_OK);
			}
	        else if (bookmarkId > 0){
                BookmarkReplaceAlertDialogFragment.newInstance(bookmarkId, 0, title, finalUrl, mCurrentFolder, true, false, false, false).show(getFragmentManager(), "replace");
                setResult(RESULT_OK);
	        } else {
	            Bitmap thumbnail;
	            Bitmap favicon;
	            ...
	            if (mSaveToHomeScreen) {
	                ...
	                if(saveFolder){
	                	bundle.putInt(EXTRA_IS_FOLDER, 1);
	                }
    ...
    public static class BookmarkReplaceAlertDialogFragment extends DialogFragment {

        public static BookmarkReplaceAlertDialogFragment newInstance(int bookmarkId, int exitId, String title, String finalUrl, long mCurrentFolder, boolean add, boolean delete, boolean move, boolean edit) {
        	BookmarkReplaceAlertDialogFragment frag = new BookmarkReplaceAlertDialogFragment();
            Bundle args = new Bundle();
            args.putInt("bookmarkId", bookmarkId);
            args.putInt("exitId", exitId);
            args.putString("title", title);
            args.putString("finalUrl", finalUrl);
            args.putLong("mCurrentFolder", mCurrentFolder);
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            final String finalUrl = getArguments().getString("finalUrl");
            final int bookmarkId = getArguments().getInt("bookmarkId");
            final long mCurrentFolder = getArguments().getLong("mCurrentFolder");
            final boolean add = getArguments().getBoolean("add");
            final boolean delete = getArguments().getBoolean("delete");
            final int exitId = getArguments().getInt("exitId");
            final boolean move = getArguments().getBoolean("move");
            final boolean edit = getArguments().getBoolean("edit");
            int dialogTitle = R.string.replace;
            int dialogMessage = R.string.add_replace_bookmark_tip;
            if(add){
                dialogTitle = R.string.add_bookmark_short;
                dialogMessage = R.string.add_replace_bookmark_tip;
            }
            if(edit){
                dialogMessage = R.string.replace_bookmark_tip;
            }
            if(move){
                dialogTitle = R.string.move_action;
                dialogMessage = R.string.move_comfire;
            }
            return new AlertDialog.Builder(getActivity()).setTitle(dialogTitle).setMessage(
                    dialogMessage)
					.setNegativeButton(R.string.cancel,
						...
						public void onClick(DialogInterface dialog, int which) {
						    if(delete){
                                ((AddBookmarkPage)getActivity()).updateBookmark(exitId, title, finalUrl, mCurrentFolder, true); 
                                ((AddBookmarkPage)getActivity()).updateBookmark(bookmarkId, title, finalUrl, mCurrentFolder, false); 
						    } else {
		                          ((AddBookmarkPage)getActivity()).updateBookmark(bookmarkId, title, finalUrl, mCurrentFolder,false);
						    }
						}
    ...
    //pekall update
	private void updateBookmark(int bookmarkId, String title, String url, long folderId, boolean delete) {
		createHandler();
//		Bundle bundle = new Bundle();
