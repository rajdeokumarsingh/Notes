...
AudioPolicyInterface
    ...
    // 如耳机plugging in/out, BT A2DPA connected/disconnected
   ...
   // retreive a device connection status
    ...
    // 如耳机连接上的时候，需要使用speaker听FM Radio，或通话中的hand free mode
    ...
    // 每次输出声音之前，都会调用这个接口(除了voice call, fm radio)
    // 一般情况，获取到的都是系统默认的output handle。 
    // BT A2DAP有一个特定的output, direct output会有特定的output
    ...
    // 声音播放完毕后，都会调用这个接口
    // 将output切换回到系统默认的设备。停止写书据在另外的流程中
